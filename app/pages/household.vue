  ...
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div class="flex items-center gap-2">
        <UButton icon="i-heroicons-arrow-left" variant="ghost" color="neutral" to="/" />
        <h1 class="text-2xl font-bold">Household</h1>
      </div>
      <UButton icon="i-heroicons-plus" label="Add Person" color="primary" @click="isAddingPerson = true" />
    </div>
    ...
    <div class="space-y-4">
      <div class="flex justify-between items-center">
        ...
        <UDropdownMenu :items="addIncomeOptions" :content="{ align: 'end' }">
          <UButton icon="i-heroicons-plus" label="Add Income" variant="subtle" color="neutral" size="sm" />
        </UDropdownMenu>
      </div>

      <div v-if="!store.config.incomeSources.length" class="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
        <p class="text-gray-500 font-medium">No income sources configured</p>
        <p class="text-sm text-gray-400">Add your TMN, salary, or Centrelink payments.</p>
      </div>

      <div v-else class="space-y-8">
        <div v-for="person in store.config.people" :key="person.id" class="space-y-3">
          <div class="flex items-center gap-2 px-1">
            <span class="text-xs font-black text-gray-400 uppercase tracking-widest">{{ person.name }}'s Income</span>
            <div class="flex-1 h-px bg-gray-100"></div>
          </div>
          
          <div class="grid grid-cols-1 gap-3">
            <IncomeSourceCard 
              v-for="source in getPersonIncomes(person.id)" 
              :key="source.id"
              :source="source"
            />
          </div>

          <div v-if="getPersonIncomes(person.id).length === 0" class="text-center py-6 bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
            <p class="text-xs text-gray-400 italic">No income sources assigned to {{ person.name }}</p>
          </div>
        </div>
      </div>
    </div>
